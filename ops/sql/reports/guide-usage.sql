-- 게임공략(도감) 사용량 점검 — 재사용용 리포트 쿼리
-- 실행: ops/sql/run.sql 에 복사해 [sql] 태그로 커밋, 또는 Supabase SQL Editor 에 붙여넣기
--
-- ⚠ 공략/도감 페이지의 "조회수" 는 이 DB 에 쌓이지 않는다 (GA4 에만 있음).
--   DB 로 알 수 있는 것: ① 공략 콘텐츠 규모 ② 공략→거래 연결 여지 ③ 비교기준(거래소 실제 조회수)
