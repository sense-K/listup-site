...
const CZN_GAME_ID = 'game_czn'

const ELEM_COLOR = {
  '질서':'#3b82f6','정의':'#f59e0b','본능':'#10b981',
  '공허':'#7c3aed','열정':'#ef4444',
}
...
<div style="max-width:560px;margin:80px auto;padding:20px;text-align:center;">
  ...
  <a href="/game/czn/characters/" style="display:inline-block;padding:12px 24px;background:#7c3aed;color:#fff;border-radius:12px;text-decoration:none;font-weight:700;">← 카오스 제로 나이트메어 캐릭터 도감</a>
</div>
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
<script src="/js/config.js"></script>
<script>document.getElementById('navbar-container').innerHTML=renderNavbar();loadAndRenderGameUI('czn')</script>
...
export async function onRequest({ params }) {
  ...
  const rows = await supaGet(
    `Character?slug=eq.${encodeURIComponent(slug)}&gameId=eq.${CZN_GAME_ID}` +
  ...
  const elemColor = ELEM_COLOR[c.element] || '#7c3aed'
  const canonical = `https://resetlist.kr/game/czn/characters/${slug}/`

  // 전신 이미지 (metadata.fullImageUrl 우선)
  const heroImg = meta.fullImageUrl || c.imageUrl

  // 같은 속성 추천
  let related = []
  if (c.element) {
    const relRows = await supaGet(
      `Character?gameId=eq.${CZN_GAME_ID}&element=eq.${encodeURIComponent(c.element)}` +
  ...
  const tradeSuffix = tradeStats.active_count > 0
    ...
    : ''
  const title = `${nameDisplay} — ${tradePrefix}카오스 제로 나이트메어 ${tierLabel} ${attrStr} 정보 | 리세리스트`
  const desc = `${c.nameKo} 캐릭터 정보, 클래스·속성 정보. 카오스 제로 나이트메어 ${tierLabel} ${attrStr}.${tradeSuffix}`
  const keywords = `${c.nameKo}, 카오스 제로 나이트메어 ${c.nameKo}, 카오스 제로 나이트메어 ${c.nameKo} 정보, 카오스 제로 나이트메어 ${tierLabel} ${c.element||''}속성, ${c.nameEn||''}`

  const relatedHtml = related.length
    ? `<div class="gc-rel-grid">${related.map(r => `
      <a class="gc-rel-card" href="/game/czn/characters/${esc(r.slug)}/">
  ...
  const jsonLd = JSON.stringify([
    { '@context':'https://schema.org','@type':'Article', headline:`카오스 제로 나이트메어 ${c.nameKo} 캐릭터 정보`, image:c.imageUrl||'', author:{name:'리세리스트'}, publisher:{name:'리세리스트',url:'https://resetlist.kr/'}, description:desc, mainEntityOfPage:canonical },
    { '@context':'https://schema.org','@type':'BreadcrumbList', itemListElement:[
      {position:1,name:'홈',item:'https://resetlist.kr/'},
      {position:2,name:'카오스 제로 나이트메어',item:'https://resetlist.kr/game/czn/'},
      {position:3,name:'캐릭터 도감',item:'https://resetlist.kr/game/czn/characters/'},
...
<head>
  ...
  <style>
    ...
    .gc-breadcrumb a:hover { color:#7c3aed; }
    ...
    .badge-5성    { background:linear-gradient(135deg,#fbbf24,#f59e0b); color:#78350f; }
    .badge-4성    { background:linear-gradient(135deg,#a78bfa,#8b5cf6); color:#fff; }
    .gc-attrs     { font-size:14px; color:#475569; margin:0 0 10px; }
    .gc-nickname  { font-size:13px; color:#7c3aed; font-weight:600; margin:0 0 10px; }
    ...
    .gc-link-primary { background:#7c3aed; color:#fff; }
...
<main class="gc-wrap">
  <nav class="gc-breadcrumb" aria-label="breadcrumb">
    ...
    <a href="/game/czn/">카오스 제로 나이트메어</a><span>›</span>
    <a href="/game/czn/characters/">캐릭터 도감</a><span>›</span>
    <span style="color:#1e293b;font-weight:600;">${esc(c.nameKo)}</span>
  </nav>

  <section class="gc-hero">
    <div class="gc-hero-img">
      ${heroImg ? `<img src="${esc(heroImg)}" alt="${esc(c.nameKo)}" fetchpriority="high" onerror="this.onerror=null;this.src='${esc(c.imageUrl||'')}';">` : ''}
    </div>
    <div>
      ...
      <p class="gc-attrs">${[c.element ? `${c.element}속성` : '', c.weaponType].filter(Boolean).map(esc).join(' · ')}</p>
      ${meta.nickname ? `<p class="gc-nickname">"${esc(meta.nickname)}"</p>` : ''}
    </div>
  </section>
  ...
  <section style="margin:32px 0;padding:28px;background:linear-gradient(135deg,#1e293b,#334155);color:#fff;border-radius:16px;box-shadow:0 8px 24px rgba(0,0,0,0.15);">
    ...
    <a href="/trade/czn/?character=${esc(slug)}" style="display:block;text-align:center;padding:14px;background:#f59e0b;color:#1e293b;font-weight:700;font-size:16px;border-radius:10px;text-decoration:none;margin-top:20px;">
  ...
  <section style="margin:32px 0;padding:28px;background:linear-gradient(135deg,#fef3c7,#fde68a);border-radius:16px;border:1px dashed #f59e0b;">
    <div style="text-align:center;">
      ...
      <a href="/trade/register/?game=czn" style="display:inline-block;padding:12px 28px;background:#f59e0b;color:#fff;font-weight:700;border-radius:10px;text-decoration:none;">판매계정 등록하기</a>
    </div>
  </section>`}

  <p class="gc-empty" style="margin:0;padding:32px 0;text-align:center;color:#94a3b8;">스킬·공명 정보 준비 중이에요.</p>
  ...
  <div class="gc-bottom-links">
    <a href="/trade/czn/"              class="gc-link gc-link-primary">🌑 카오스 제로 나이트메어 거래소</a>
    <a href="/game/czn/characters/"    class="gc-link gc-link-sec">← 캐릭터 도감</a>
    <a href="/trade/price/czn/"        class="gc-link gc-link-sec">📊 카오스 제로 나이트메어 시세</a>
  </div>
</main>
