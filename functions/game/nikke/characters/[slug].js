...
const NIKKE_GAME_ID = 'cmnile0mk0003g8cv1pczc9d7'

const ELEM_COLOR = {
  'Fire': '#ef4444', 'Water': '#3b82f6', 'Wind': '#10b981',
  'Electric': '#fbbf24', 'Iron': '#6b7280',
}
const ELEM_KO   = { Fire:'화염', Water:'수속', Wind:'풍속', Electric:'전기', Iron:'철갑' }
const WEAPON_KO = { Shotgun:'샷건', SMG:'SMG', 'Assault Rifle':'어설트 라이플', Minigun:'미니건', 'Rocket Launcher':'로켓 런처', 'Sniper Rifle':'스나이퍼 라이플' }
const MFR_KO   = { Elysion:'엘리시온', Missilis:'미사일리스', Tetra:'테트라', Pilgrim:'필그림', Abnormal:'콜라보' }
...
<div style="max-width:560px;margin:80px auto;padding:20px;text-align:center;">
  ...
  <a href="/game/nikke/characters/" style="display:inline-block;padding:12px 24px;background:#c026d3;color:#fff;border-radius:12px;text-decoration:none;font-weight:700;">← 니케 캐릭터 도감</a>
</div>
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
<script src="/js/config.js"></script>
<script>
  document.getElementById('navbar-container').innerHTML = renderNavbar()
  loadAndRenderGameUI('nikke')
</script>
</body>
...
export async function onRequest({ params }) {
  ...
  const rows = await supaGet(
    `Character?slug=eq.${encodeURIComponent(slug)}&gameId=eq.${NIKKE_GAME_ID}` +
    `&isActive=eq.true&select=id,nameKo,nameEn,rarity,element,weaponType,burstType,manufacturer,classType,isLimited,slug,imageUrl,metadata&limit=1`
  )
  if (!rows?.[0]) return respond404(`'${esc(slug)}' 캐릭터를 찾을 수 없어요.`)

  const c = rows[0]
  const meta = (typeof c.metadata === 'object' && c.metadata !== null) ? c.metadata : {}
  const isTreasure = slug.endsWith('-treasure')
  const rarityLabel = c.rarity || 'SSR'
  const elemColor = ELEM_COLOR[c.element] || '#c026d3'
  const elemKo    = ELEM_KO[c.element]   || c.element || ''
  const weaponKo  = WEAPON_KO[c.weaponType] || c.weaponType || ''
  const mfrKo     = MFR_KO[c.manufacturer]  || c.manufacturer || ''
  const canonical = `https://resetlist.kr/game/nikke/characters/${slug}/`

  // 같은 속성 추천
  let related = []
  if (c.element) {
    const relRows = await supaGet(
      `Character?gameId=eq.${NIKKE_GAME_ID}&element=eq.${encodeURIComponent(c.element)}` +
      `&id=neq.${c.id}&isActive=eq.true&select=nameKo,slug,imageUrl,rarity&limit=8`
    )
  ...
  const attrStr = [elemKo ? `${elemKo}속성` : '', weaponKo].filter(Boolean).join(' ')
  ...
  const tradeSuffix = tradeStats.active_count > 0
    ...
    : ''
  const title = `${nameDisplay} — ${tradePrefix}니케 ${rarityLabel}급 ${attrStr} 정보 | 플레이센스`
  const desc = [
    `${c.nameKo} 캐릭터 정보, 등급, 속성, 무기, 제조사, 버스트 정보.`,
    ` 니케 ${rarityLabel} ${elemKo ? elemKo + '속성' : ''} ${weaponKo}`,
    mfrKo ? ` ${mfrKo} 소속.` : '.',
    isTreasure ? ' 애장품 강화 버전.' : '',
  ].join('') + tradeSuffix
  const keywords = `${c.nameKo}, 니케 ${c.nameKo}, 니케 ${c.nameKo} 정보, 니케 ${rarityLabel} ${elemKo}속성, ${c.nameEn || ''}`

  const jsonLdArr = [
    {
      '@context': 'https://schema.org', '@type': 'Article',
      headline: `니케 ${c.nameKo} 캐릭터 정보`,
      image: c.imageUrl || '',
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: '니케', item: 'https://resetlist.kr/game/nikke/' },
        { '@type': 'ListItem', position: 3, name: '캐릭터 도감', item: 'https://resetlist.kr/game/nikke/characters/' },
        { '@type': 'ListItem', position: 4, name: c.nameKo },
      ],
    },
  ]

  // 스킬 (metadata.skills 있으면 표시)
  const skillsHtml = meta.skills?.length
    ? meta.skills.map((s, i) => `
      <details class="gc-skill"${i === 0 ? ' open' : ''}>
        <summary class="gc-skill-head">
          <span class="gc-skill-type">${esc(s.type || '스킬')}</span>
  ...
  // 추가 정보 테이블
  const infoRows = [
    c.rarity    ? `<tr><th>등급</th><td>${esc(c.rarity)}</td></tr>` : '',
    c.classType ? `<tr><th>역할</th><td>${esc(CLASS_KO[c.classType] || c.classType)}</td></tr>` : '',
    c.burstType ? `<tr><th>버스트</th><td>버스트 ${esc(c.burstType)}</td></tr>` : '',
    mfrKo       ? `<tr><th>제조사</th><td>${esc(mfrKo)}</td></tr>` : '',
  ].filter(Boolean).join('')

  // 등급 뱃지 색상
  const rarityBadgeStyle = c.rarity === 'SSR'
...
<head>
  ...
  <style>
    ...
    .gc-breadcrumb a:hover { color:#c026d3; }
    ...
    .gc-info-table { width:100%; border-collapse:collapse; font-size:13px; margin-top:12px; }
    .gc-info-table th { text-align:left; color:#94a3b8; font-weight:600; padding:4px 12px 4px 0; width:70px; white-space:nowrap; }
    ...
    .gc-rarity-badge { position:absolute; top:5px; left:5px; font-size:9px; font-weight:800; padding:2px 5px; border-radius:4px; background:rgba(0,0,0,0.5); color:#fff; }
    ...
    .gc-link-primary { background:#c026d3; color:#fff; }
...
<main class="gc-wrap">
  ...
  <nav class="gc-breadcrumb" aria-label="breadcrumb">
    ...
    <a href="/game/nikke/">니케</a><span>›</span>
    <a href="/game/nikke/characters/">캐릭터 도감</a><span>›</span>
  ...
  <section class="gc-hero">
    ...
    <div>
      ...
      <div class="gc-badges">
        <span class="gc-badge" style="${rarityBadgeStyle}">${rarityLabel}급</span>
        ${elemKo ? `<span style="font-size:13px;font-weight:700;color:${elemColor};">${esc(elemKo)}속성</span>` : ''}
        ${isTreasure ? `<span style="font-size:12px;font-weight:700;padding:3px 8px;border-radius:6px;background:#f3e8ff;color:#7c3aed;">💎 애장품</span>` : ''}
        ${c.isLimited ? `<span style="font-size:12px;font-weight:700;padding:3px 8px;border-radius:6px;background:#fef3c7;color:#92400e;">🌟 한정</span>` : ''}
      </div>
      <p class="gc-attrs">${[elemKo ? `${elemKo}속성` : '', weaponKo, c.burstType ? `버스트 ${c.burstType}` : '', mfrKo].filter(Boolean).map(esc).join(' · ')}</p>
  ...
  <section class="trade-widget" style="margin:32px 0;padding:28px;background:linear-gradient(135deg,#1e293b,#334155);color:#fff;border-radius:16px;box-shadow:0 8px 24px rgba(0,0,0,0.15);">
    ...
    <a href="/trade/nikke/?character=${esc(slug)}"
  ...
  <section style="margin:32px 0;padding:28px;background:linear-gradient(135deg,#fef3c7,#fde68a);border-radius:16px;border:1px dashed #f59e0b;">
    <div style="text-align:center;">
      ...
      <a href="/trade/register/?game=nikke"
  ...
  ${related.length ? `
  <h2 class="gc-h2">같은 ${esc(elemKo || '')}속성 니케</h2>
  ${relatedHtml}` : ''}

  <div class="gc-bottom-links">
    <a href="/trade/nikke/"              class="gc-link gc-link-primary">🔫 니케 거래소</a>
    <a href="/game/nikke/characters/"    class="gc-link gc-link-sec">← 캐릭터 도감</a>
    <a href="/trade/price/nikke/"        class="gc-link gc-link-sec">📊 니케 시세</a>
  </div>
