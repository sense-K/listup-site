...
const WUWA_GAME_ID = '902fe427-47d2-4a3c-88e5-04218bad66c6'

const ELEM_COLOR = {
  '응결':'#3b82f6','융해':'#ef4444','전도':'#9333ea',
  '기류':'#10b981','회절':'#f59e0b','인멸':'#9c27b0',
}
...
<div style="max-width:560px;margin:80px auto;padding:20px;text-align:center;">
  ...
  <a href="/game/wuwa/characters/" style="display:inline-block;padding:12px 24px;background:#06b6d4;color:#fff;border-radius:12px;text-decoration:none;font-weight:700;">← 명조 캐릭터 도감</a>
</div>
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
<script src="/js/config.js"></script>
<script>document.getElementById('navbar-container').innerHTML=renderNavbar();loadAndRenderGameUI('wuwa')</script>
...
export async function onRequest({ params }) {
  ...
  const rows = await supaGet(
    `Character?slug=eq.${encodeURIComponent(slug)}&gameId=eq.${WUWA_GAME_ID}` +
  ...
  const canonical = `https://resetlist.kr/game/wuwa/characters/${slug}/`
  ...
  if (c.element) {
    const relRows = await supaGet(
      `Character?gameId=eq.${WUWA_GAME_ID}&element=eq.${encodeURIComponent(c.element)}` +
  ...
  const relatedHtml = related.length
    ? `<div class="gc-rel-grid">${related.map(r => `
      <a class="gc-rel-card" href="/game/wuwa/characters/${esc(r.slug)}/">
  ...
  const jsonLd = JSON.stringify([
    ...
    { '@context':'https://schema.org','@type':'BreadcrumbList', itemListElement:[
      ...
      {position:3,name:'캐릭터 도감',item:'https://resetlist.kr/game/wuwa/characters/'},
...
<head>
  ...
  <style>
    ...
    .gc-nickname  { font-size:13px; color:#06b6d4; font-weight:600; margin:0 0 10px; }
...
<main class="gc-wrap">
  <nav class="gc-breadcrumb" aria-label="breadcrumb">
    ...
    <a href="/game/wuwa/">명조 워더링 웨이브</a><span>›</span>
    <a href="/game/wuwa/characters/">캐릭터 도감</a><span>›</span>
  ...
  <section style="margin:32px 0;padding:28px;background:linear-gradient(135deg,#1e293b,#334155);color:#fff;border-radius:16px;box-shadow:0 8px 24px rgba(0,0,0,0.15);">
    ...
    <a href="/trade/wuwa/?character=${esc(slug)}" style="display:block;text-align:center;padding:14px;background:#f59e0b;color:#1e293b;font-weight:700;font-size:16px;border-radius:10px;text-decoration:none;margin-top:20px;">
  ...
  <section style="margin:32px 0;padding:28px;background:linear-gradient(135deg,#fef3c7,#fde68a);border-radius:16px;border:1px dashed #f59e0b;">
    <div style="text-align:center;">
      ...
      <a href="/trade/register/?game=wuwa" style="display:inline-block;padding:12px 28px;background:#f59e0b;color:#fff;font-weight:700;border-radius:10px;text-decoration:none;">판매계정 등록하기</a>
    </div>
  </section>`}
  ...
  <div class="gc-bottom-links">
    <a href="/trade/wuwa/"              class="gc-link gc-link-primary">🌊 명조 거래소</a>
    <a href="/game/wuwa/characters/"    class="gc-link gc-link-sec">← 캐릭터 도감</a>
    <a href="/trade/price/wuwa/"        class="gc-link gc-link-sec">📊 명조 시세</a>
  </div>
</main>
